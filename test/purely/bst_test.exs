defmodule Purely.BSTTest do
  ...
  test "put_new" do
    ptest xs: list(of: int(min: 1, max: 5000)) do
      ...
      bst = Enum.reduce(xs ++ ys, BST.new, fn {k,v}, acc ->
        BST.put_new(acc, k, v)
      end)
      assert BST.inorder(bst) == Enum.uniq(Enum.sort(xs))
    end
  ...
  test "put_new_lazy" do
    ptest xs: list(of: int(min: 1, max: 5000)) do
      bst = BST.new
      bst = Enum.reduce(xs, bst, fn x, acc ->
  ...
  test "update missing" do
    ptest xs: list(of: int(min: 1)),
      ...
      bst = Enum.reduce(Enum.uniq(missing), BST.new(Enum.zip(xs, xs)), fn y, acc ->
  ...
  test "get_and_update missing" do
    ptest xs: list(of: int(min: 1)),
      missing: list(of: int(max: -1)) do
      bst = BST.new(Enum.zip(xs, xs))
      bst = Enum.reduce(Enum.uniq(missing), bst, fn y, acc ->
  ...
  test "merge with callback" do
    ...
    ptest xs: list(of: int(min: 1)),
      ...
      bst1 = BST.new(Enum.zip(xs, xs))
      bst2 = BST.new(Enum.zip(ys, ys))
      merged = BST.merge(bst1, bst2, fn _,_,_ ->
  ...
  test "drop" do
    ptest xs: list(of: int()),
      ...
      assert BST.inorder(dropped_bst) == BST.inorder(BST.new(Enum.zip(undropped_keys, undropped_keys)))
    end
  end
